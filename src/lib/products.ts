...
export async function getAllProducts(limit: number = 10): Promise<Product[] | null> {
   const query = gql`
   ...
   try {
      ...
      const response = await graphqlClient.request<{ products: { edges: { node: RawProduct }[] } }>(query, variables);
